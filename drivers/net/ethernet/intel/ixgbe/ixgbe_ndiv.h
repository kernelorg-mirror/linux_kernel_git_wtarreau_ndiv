...
void ixgbe_ndiv_send_rsp(struct ixgbe_q_vector *q_vector) {
	...
	if (!netif_xmit_frozen_or_stopped(txring_txq(q_vector->tx.ring))) {
		while (likely(q_vector->ndiv_rsp.pending && ixgbe_desc_unused(q_vector->tx.ring) >= 1)) {
			...
			if (unlikely(ixgbe_desc_unused(q_vector->tx.ring) == 1 || q_vector->ndiv_rsp.pending == 1))
			...
			dma_unmap_len_set(tx_buffer, len, sizeof(packet->data));
			dma_unmap_addr_set(tx_buffer, dma, packet->dma);
